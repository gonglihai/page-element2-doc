...
`search` 查询条件的配置是一个数组或者一个对象。

当为数组时，数组中的每一项代表一个查询条件；

当为对象时，对象的 `item` 属性代表查询条件数组。

查询条件的类型通过 `type` 属性进行区分。以下是配置示例：
...
> [!NOTE] 
>
> 查询条件的配置最初为数组类型，后因需增加属性而更改为对象类型。为兼容历史代码，当前同时支持数组和对象两种形式。
...
| clear  | 清空查询 (重置查询)，不会触发 `search` 事件。 | -    |
